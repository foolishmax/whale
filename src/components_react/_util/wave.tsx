...
export default class Wave extends React.Component<WaveProps> {
  ...
  onClick = (node: HTMLElement, waveColor: string) => {
    ...
    if (
      ...
      [${getPrefixCls(
        ...
        --whale-wave-shadow-color: ${waveColor};
      }`,
        'whale-wave',
    ...
    ['transition', 'animation'].forEach((name) => {
      (node as any).addEventListener(`${name}start`, this.onTransitionStart);
      (node as any).addEventListener(`${name}end`, this.onTransitionEnd);
    });
  };
  ...
  resetEffect(node: HTMLElement) {
    ...
    ['transition', 'animation'].forEach((name) => {
      (node as any).removeEventListener(`${name}start`, this.onTransitionStart);
      (node as any).removeEventListener(`${name}end`, this.onTransitionEnd);
    });
  }
